...
var authored = []Doc{
	...
	{ID: "cricket-ai", Title: "Cricket AI: artificial intelligence for cricket fans",
		Text: "A cricket AI is an artificial-intelligence assistant that understands the game and explains it on demand. Cricket for Noobs is a cricket AI built for American sports fans: it watches live matches — scores, ball-by-ball commentary, player form, and betting-market prices — and answers any question in baseball terms, from 'who's winning?' to 'what is a yorker?'. It speaks answers aloud, narrates games live, and never invents numbers: everything exact is computed from real data."},
	{ID: "cricket-llm", Title: "Cricket LLM: an AI language model for cricket",
		Text: "Cricket for Noobs is an LLM for cricket: a large language model assistant that answers cricket questions in plain English with baseball translations. Unlike a general chatbot, it is grounded in live data — real-time scores, ball-by-ball commentary, player statistics, and prediction-market prices — so answers about a live match reflect what is actually happening. Exact facts (scores, odds, definitions) are computed deterministically; the LLM writes only the explanation around them."},
